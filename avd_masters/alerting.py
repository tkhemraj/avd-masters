...
def get_email_config() -> dict:
    ...
    return {
        ...
        "to_addrs": [a.strip() for a in os.getenv("AVD_ALERT_TO", "").split(",") if a.strip()] if os.getenv("AVD_ALERT_TO") else [],
...
def send_funky_gpu_email(
    ...
    <html>
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.5;">
        ...
        <p><strong>Summary:</strong> {_html_escape(summary)}</p>
        <h4>Details</h4>
        <ul>
          {''.join(f'<li>{_html_escape(d)}</li>' for d in details)}
        </ul>
        <h4>Recommended Actions</h4>
        <ul>
          {''.join(f'<li>{_html_escape(r)}</li>' for r in recommendations)}
        </ul>
        {f'<p><strong>Impact:</strong> {_html_escape(impact)}</p>' if impact else ''}
